...
6. **Run the `check:docs` gate (`MATERIA.md § Gate`), then commit and hand off to `docs-audit`.**
   ...
   `${CLAUDE_PLUGIN_ROOT}/skills/docs-audit/SKILL.md`.
...
- the `check:docs` gate (`MATERIA.md § Gate`) **passes — run it yourself in step 6** (links + style;
...
the `check:docs` gate (`MATERIA.md § Gate`) to catch link + style issues mechanically, or invoke the
