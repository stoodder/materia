---
name: materia-implement-task
description: Implement one task from a tasks.md — read context, build to the standards, write tests, then run the local gate and commit. Stage 6 of the ship-spec pipeline; called once per task. Adversarial review happens after all implementation tasks complete (see materia-ship-spec/SKILL.md § Review).
---

# materia-implement-task — build one task to done

Take a single task to a clean, tested, committed state. Runs as a
subagent in `materia-ship-spec` (one per task; independent tasks may run in parallel,
worktree-isolated); usable standalone.

Adversarial review is **not** per-task in this pipeline. After every task
finishes, `materia-ship-spec` runs a single fresh-context review pass over the
cumulative branch diff (see `materia-ship-spec/SKILL.md` § Review) — that is where
...
3. **Implement** to the standards and the Definition of Done
   ...
   **UI work matches its cohesion anchors.** When the task touches UI, read
   ...
   comes from reuse, not from re-deriving tokens per screen; `materia-ui-review`
...
5. **Task gate.**
   ...
   **Gate.** `lint` and `typecheck` (`MATERIA.md` § Gate) clean and the
   ...
   post-implementation review pass (see `materia-ship-spec/SKILL.md` § Review);
...
6. **Persist:** update **directly-touched** docs in the same change. Use
   ...
   to compute the set; if `materia-plan-tasks` tagged this task with a docs-scope
   ...
   `materia-finalize` → `materia-docs-sync` to avoid cross-task drift. Then set the task to
   ...
   so `materia-finalize` doesn't have to reconcile them. Tick progress in `STATUS.md`
   (set `Next` to the
   following task; add the task ID to `behavior-deferred:` if any AC is
   user-visible so the post-implementation review and `materia-finalize`'s verify
   rerun see it). **Commit + push.**

   **Orchestrator-lane exception (no `STATUS.md`/`retro.md` writes).** When this
   task runs **spawned by `materia-ship-spec`** (the orchestrator lane), the orchestrator
   ...
   **superseded**: tick **only** this task's `tasks.md` acceptance-criteria boxes
   ...
   and records `behavior-deferred:` after you return (see `materia-ship-spec/SKILL.md`
   ...
   **Return your retro entry — do not touch `retro.md`.** Close your report
   ...
   from `materia-ship-spec/SKILL.md` § Retrospective capture. Stamp the entry's
...
- Directly-touched docs updated (cross-cutting deferred to `materia-docs-sync`); task
