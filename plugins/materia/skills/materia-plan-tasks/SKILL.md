...
# materia-plan-tasks — architecture → ordered tasks

Break `architecture.md` into tasks the `materia-implement-task` skill can each finish
end-to-end (code + tests + review + docs). Runs as a subagent in `materia-ship-spec`;
usable standalone.
...
3. **Specify each task**: id, **depends-on**, **model/effort**, area/files, scope, **testable
   ...
   **Model/effort.** Assign each task a `<model>/<effort>` pair by matching the
   ...
   - **Any cross-cutting doc** (`CLAUDE.md`, `docs/README.md` index,
     ...
     `materia-docs-sync`" is overridden when architecture has already named the
   ...
   - **Rule of thumb: fix staleness in files this branch modifies; defer only
     ...
     doc describes, that doc is in-floor — don't push it to `materia-docs-sync`. Two
   ...
   - **Enumerate every depiction of a changed structure, not just the
     ...
     `/materia-ship-spec` mermaid, a second bug-flow chart) can't silently drift. And
   ...
   `materia-implement-task` treats this list as a floor and unions it with what it
   derives from the actual diff at commit time. The `materia-finalize` → `materia-docs-sync`
   pass still runs to catch cross-task drift or missed touch-Y rows; the
   per-task floor is the proactive layer, `materia-docs-sync` is the safety net.

   **Cross-reference: `materia-implement-task`'s hard doc-exclusion list.** When
   composing a floor, read it against `materia-implement-task/SKILL.md` § Persist
   ...
   `materia-finalize` → `materia-docs-sync`. A floor entry for a cross-cutting doc is only
...
- **Renames run a whole-repo grep before slicing.** For any rename (a model,
  ...
  **whole-repo case-insensitive grep** of the old token (`git grep -iI '<old>'`)
  ...
  and its hit count so `materia-implement-task` and the reviewer can re-run it.
...
- **A task that renames or changes a source file owns that file's tests.**
  ...
  **in the same task**, so every task leaves its relevant tests green and is
  ...
  `materia-implement-task`'s "relevant tests green" gate and creates an artificial
...
- **Docs-scope floor is usually `[]`.** Cross-cutting docs (`CLAUDE.md`,
  ...
  `materia-finalize` → `materia-docs-sync` exactly as in product runs.
- **`Tests:` field can be `none — verification is read-against-spec`.**
  ...
  `materia-implement-task/SKILL.md` step 5 markdown exemption).
