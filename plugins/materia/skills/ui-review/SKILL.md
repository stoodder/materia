...
0. **UI self-gate (no-op in the orchestrated lane).** Before provisioning
   ...
   `ui-review: skipped (no UI surface — § UI-affecting is none)` — and end
   cleanly, writing nothing (no Eyes provisioning, no findings). This gate is a
   no-op in the orchestrated lane: `ship-spec` only spawns this angle on a
...
1. **Provision the Eyes environment** — run the provisioning recipe from
   ...
   `${CLAUDE_PLUGIN_ROOT}/skills/ship-spec/resources/env-preflight.md` § Gate invocation
   notes).
