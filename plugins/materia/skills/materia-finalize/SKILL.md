...
`materia-ship-spec`; usable standalone once tasks are done. **Orchestrator-lane
...
that masks a real failure (see step 1 and `materia-ship-spec/SKILL.md`
...
1. **Behavior re-check for deferred tasks.** Read `behavior-deferred:` from
   ...
   **foreground with explicit exit-code capture**, never `nohup … &` — a
   ...
   failure (see `materia-ship-spec/SKILL.md` § Orchestrator behavioral-verify lane).
   **Retry up to 2x** on
...
2. **Run the full gate** from the repo root and **fix everything it surfaces**
   ...
   **docs/skills-only gate profile.** When the cumulative diff is **zero-code
   ...
   same predicate as `materia-ship-spec` § Review's markdown-only exemption) — the
...
3''. **E2e-coverage gate (UI-affecting features only).** Check whether this
   ...
   `materia-ship-spec/SKILL.md` § Review — § UI-surface gate (do NOT re-enumerate the
...
4. **Open the PR.** **Shell hygiene for title/body:** never interpolate
   ...
   **UI proof block in the PR body.** This block is gated by the same
   ...
   `materia-ship-spec/SKILL.md` § Review — § UI-surface gate) — do NOT re-enumerate its
   ...
   - **UI-affecting diff, `ui-proof/` present and contains PNGs (ready):**
     ...
     **URL construction (new — no existing precedent in the skills; follow
     ...
     1. `<sha>` = `git rev-parse HEAD` at the moment finalize renders the PR
        body. The `materia-ui-review` `ui-proof/` commit is already on the branch at
        this point (it was committed by `materia-ui-review` during the review stage,
   ...
   - **UI-affecting diff, `ui-proof/` absent or empty (or all files
     ...
     (`materia-ship-spec/SKILL.md` § Review — § Screenshot-presence check), so this
     ...
     - **Reason recorded** — one of:
       ...
       because `materia-ui-review` never ran under the `ui-coverage-waiver` path
...
backgrounded subagent can hold (see `materia-ship-spec/SKILL.md` § Orchestrator
...
- **Commit orchestrator-lane ui-proof screenshots to the branch before the PR
  body is rendered.** When `materia-ui-review` ran in the orchestrator lane, its
