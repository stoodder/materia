...
walker. The shape is fixed by `materia-triage-retros`'s `## bug-reports.md
...
`materia-triage-retros` placed in the `bugs[]` bucket with an assigned severity. The
...
6. **Open the PR** with `gh pr create`. Title:
   ...
   change). Closing line: "Work each report with `/materia-fix-bug`."
...
Queue these reports with:  /materia-fix-bug
```
...
`materia-triage-retros`, `materia-report-bug`, and `materia-intake-spec` use). `source_refs` is
...
- Does NOT run `/materia-fix-bug` or implement any product change. After the PR lands,
  the operator runs `/materia-fix-bug` to work each report.
...
- Does NOT cluster or filter items — items emitted by `materia-triage-retros` are
...
- **Defaults beat questions.** Parse items and draft reports in one shot using
  ...
  step 4.
- **No cluster/filter pass.** Bug items are pre-triaged by `materia-triage-retros`;
...
- The renderer always emits every required H2 verbatim so a consumer can strip
  the frontmatter and pass the body directly to `/materia-fix-bug`.
