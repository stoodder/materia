---
name: materia-apply-pipeline-improvements
description: The executor half of the triage-retros split. Run manually (schedulable later) after `materia-triage-retros` has landed plans. Globs `docs/specs/_improvements/**/pipeline-improvements.md` for unprocessed improvement plans, builds a dimension-tagged candidate set from each plan's `## Actions`, runs a keep–supersede–conflict Pareto selection pass (surfacing any conflicts to the operator before any edits land), applies the selected deltas to the `materia-ship-spec` pipeline skills with per-delta anti-regression notes, has a fresh-context subagent review the diff against the plan, opens one PR per plan (no auto-merge) with a keep/supersede/conflict disposition summary, and renames the consumed plan to `pipeline-improvements.processed.md` in the same PR. Consumes `pipeline-improvements.md` only — disjoint inputs and edit-paths from `materia-suggestions-to-specs` (which consumes `product-suggestions.md`), so the two run in parallel. Idempotent via the rename; a failed run leaves the plan unprocessed for retry.
---

# materia-apply-pipeline-improvements — apply the planned pipeline edits

The executor half of the two-skill `materia-triage-retros` split. `materia-triage-retros`
...
`materia-ship-spec` pipeline skills, has a fresh-context subagent review the result,
...
It runs **independently of `materia-suggestions-to-specs`**: that skill consumes
...
Read `.claude/skills/materia-triage-retros/resources/actions-contract.md` (the
...
`materia-ship-spec` pipeline skills under `.claude/skills/` (the edit targets — this
...
`materia-triage-retros/resources/actions-contract.md` (shape truth: the `_templates/`
stub).
...
`materia-triage-retros/resources/actions-contract.md` § Dimension vocabulary (the
...
`materia-ship-spec/SKILL.md` about resuming after a crash, `dimension:
...
**Worked example C — independent keep.** The plan has A1 (`materia-ship-spec/SKILL.md`,
`dimension: review-precision`) and A4 (`materia-docs-sync/SKILL.md`, `dimension:
...
The operator re-runs `materia-triage-retros` after merge to produce a follow-up plan
...
`.claude/skills/materia-ship-spec/SKILL.md`, run the section-scoped diff (see
...
|---|---|
| `docs/specs/_templates/retro.md` | The schema `materia-triage-retros`'s parser is built against. |
| `.claude/skills/materia-ship-spec/SKILL.md` § Retrospective capture | The retro-generation contract — what gets written into future retros. |
| `.claude/skills/materia-triage-retros/resources/actions-contract.md` / `resources/rendering.md` | The artifact contracts this skill, `materia-suggestions-to-specs`, and `materia-bugs-to-reports` parse. |
...
### Layer 2 — section-scoped diff for `materia-ship-spec/SKILL.md`

The allowlist can't tell "editing `materia-ship-spec/SKILL.md` § Retrospective capture"
from "editing other parts of `materia-ship-spec/SKILL.md`" — the whole file matches
the first allowlist row. So for **any** edit to that file, run a section-scoped
diff before committing the action:

```bash
git show HEAD:.claude/skills/materia-ship-spec/SKILL.md \
  | awk '/^## Retrospective capture/{flag=1} /^## /{if(flag && !/^## Retrospective capture/) exit} flag' \
  > /tmp/appi-base
awk '/^## Retrospective capture/{flag=1} /^## /{if(flag && !/^## Retrospective capture/) exit} flag' \
  .claude/skills/materia-ship-spec/SKILL.md \
  > /tmp/appi-head
...
Discard the change (`git checkout -- .claude/skills/materia-ship-spec/SKILL.md`) and
...
`materia-triage-retros` for a re-draft. Re-run the review only if the changes were
...
5. **Conflicts excluded — re-draft required** (omit when there are no
   ...
   `materia-triage-retros` to produce a plan that resolves the conflict explicitly:
   ...
   - A3 vs A7: review-precision vs token-cost on `materia-intake-spec/SKILL.md` §
     intake prompt — both edits modify the same prose with incompatible
     direction.

   Re-run `materia-triage-retros` to produce a follow-up plan that resolves these
...
- **Consumes `pipeline-improvements.md` only.** Does NOT touch `product-suggestions.md`, the
  ...
  the consumed plan file — disjoint from `materia-suggestions-to-specs`, so the two run
  in parallel.
...
- **Smaller and well-scoped beats sweeping.** Favor a set of changes the
  ...
  signal the action belongs back in `materia-triage-retros` (or its own `materia-ship-spec`
...
- **Faithful, not creative.** Apply the plan; don't add unrequested
  ...
  retro/plan input for `materia-triage-retros`, not a freelance edit here.
