...
# materia-report-bug — capture a bug, produce a report file
...
|---|---|
| `/materia-report-bug <description text>` | Use the trailing text as the bug description, advance. |
| `/materia-report-bug` (no args) + AskUserQuestion available | Ask "Describe the bug — rough notes are fine; I'll fill in the rest." |
| `/materia-report-bug` (no args) + Auto Mode (no AskUserQuestion) | Print the same prompt and end the turn. The next user reply is the description. |
...
`materia-intake-spec` and `materia-triage-retros` use for `<rand>`).
