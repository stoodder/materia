...
# materia-intake-spec — normalize a raw spec
...
Runs as a subagent in `materia-ship-spec`; usable standalone.
...
- **The spec body** — either the raw freeform spec / feature request from
  ...
  passed in by `materia-ship-spec` after it stripped the frontmatter from a
...
- **Optional: `pre-created-folder: docs/specs/<dated-slug>/`** —
  `materia-ship-spec` signals this when it has already minted the dated-slug,
...
2. **Mint the `<dated-slug>` (skipped on the proposal path).** Per the rule
   ...
   **If the orchestrator passed `pre-created-folder:`**, skip this step
   entirely — `materia-ship-spec` has already minted the folder and seeded
   ...
   **Do NOT seed `retro.md`.** The `materia-ship-spec` orchestrator owns the retro
   file and creates it immediately after intake returns (see
   `materia-ship-spec/SKILL.md` § Retrospective capture). If you're running intake
...
3. **Detect the input shape.** Before drafting, check whether the body is
   ...
   certainly a proposal body that `materia-ship-spec` stripped from a
...
4. **Find the gaps and ASK.** Anything ambiguous or unstated that changes scope,
   ...
   **Auto-Mode path when `AskUserQuestion` is unavailable.** In practice this is
   ...
   AND you are running under the `materia-ship-spec` orchestrator, treat that as
   **Auto Mode active**. If you are running standalone (no orchestrator) and
   ...
   3. **Return outcome `partial`** to the caller in the subagent return
      ...
      needed before the next stage." The `materia-ship-spec` orchestrator
      intercepts `partial` and surfaces the Open questions to the human
      before spawning `materia-design` (see `materia-ship-spec/SKILL.md` § Intake hand-off
      for `partial` outcome).
...
branch (step 3) is reserved for `materia-ship-spec`'s proposal path.
