...
decomposes into one member spec is `materia-propose-spec` with extra steps.

### 2. Read project context (silently, before any questions)

Same read set as `materia-propose-spec` step 2 (always: `CLAUDE.md`,
...
- Validate identifiers + freshness against the live codebase per
  `materia-propose-spec` step 2 (grep intended names, already-shipped scan, stable
  sibling paths).

### 3. Brainstorm Q&A (iterative, with the operator)

Unlike `materia-propose-spec`, this step is a deliberate multi-round conversation.
Per round:
...
must be **self-contained** — a `materia-ship-spec` run sees only the proposal body,
...
Everything from the queue contract and `materia-propose-spec`'s § File format holds
...
`materia-intake-spec`'s required-H2 detector sees the standard spine first:
...
- Does NOT maintain the epic after this run — that's
  [`materia-reconcile-epic`](../materia-reconcile-epic/SKILL.md)'s job.
...
- **Always ends in a PR** on `approve` — same unconditional path as
  `materia-propose-spec` (branch → write → commit → push → PR); never stop at a
  pushed branch.
