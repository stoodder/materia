...
# materia-ui-test-plan — enumerate UI flows and write the test plan

Read `spec.md` + `design.md`, identify the UI flows worth guarding, and write a
`ui-test-plan.md` artifact the downstream `materia-plan-tasks` stage consumes to derive
the e2e-authoring task. Runs as a subagent in `materia-ship-spec` (UI-gated — only
spawned when the run is UI-affecting; the gate is defined in
`materia-ship-spec/SKILL.md` § Review — § UI-surface gate, whose predictive form is
evaluated once after intake and covers both `materia-design` and this stage); usable
standalone after `materia-design` returns.
...
5. **Persist:** tick stage 3 in `STATUS.md` and set `Next: architecture`;
   commit + push. **Orchestrator-lane exception:** when spawned by `materia-ship-spec`/`materia-fix-bug`, do **not** tick `STATUS.md` or commit it — the orchestrator owns both (see `materia-ship-spec/SKILL.md` § STATUS.md ownership (orchestrator lane)); write only your own artifact.
...
- Write e2e test code — that is the e2e-authoring task `materia-plan-tasks`
...
- Evaluate the UI-surface gate itself — the orchestrator evaluates
  `materia-ship-spec/SKILL.md` § Review — § UI-surface gate before spawning this skill;
  this skill assumes it has been gated in.

## Standalone use

After `materia-design` has committed `design.md`, invoke this skill with the spec's
`<dated-slug>` to produce `ui-test-plan.md` without running a full `materia-ship-spec`
pipeline. Verify `design.md` exists before running. The skill commits and pushes
`ui-test-plan.md` + the STATUS tick, leaving the repo ready for the `materia-architecture`
stage.
