...
# materia-reconcile-epic — cascade shipped reality back through an epic

The maintainer half of the epic family
([`materia-propose-epic`](../materia-propose-epic/SKILL.md) is the creator half). Epics are
plans made before any member ships; each `materia-ship-spec` run then produces
...
- **Pipeline mode** — spawned by `materia-ship-spec` between docs-audit and finalize
  whenever the run's proposal is an epic member (the § Epic gate in
  `materia-ship-spec/SKILL.md`). The sync + cascades commit on the run's existing
...
- **Standalone mode** — invoked by the operator as `/materia-reconcile-epic`. The
  backstop and status board: for drift from work that bypassed the pipeline
  (manual changes, `materia-fix-bug` runs touching epic ground, a run whose epic
...
- Does NOT create new member proposals. Shipped reality revealing a missing
  ...
  `/materia-propose-spec` (or a follow-up `/materia-propose-epic` amendment PR by hand).
...
- **Cascade conservatively.** Only factual invalidation edits a pending
  ...
  editing. A wrong cascade silently corrupts a future `materia-ship-spec` run's
  input.
