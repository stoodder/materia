...
<!-- Filled by `ship-spec` at proposal selection (the run's entry point).
     ...
     from `.materia/docs/specs/_proposed/` in the same PR. For ad-hoc runs (no
...
- **Proposed-spec:** <`.materia/docs/specs/_proposed/<filename>` at selection time, or `—`>
...
<!-- CANONICAL NOTE — the two stage-numbering scales (single source of truth).
     ...
     cross-reference it ("stage numbering: see .materia/docs/specs/_templates/status.md
