...
`.materia/docs/bugs/` is the **bug-report tree** — a sibling to `.materia/docs/specs/`. It holds the
queue contract, the report body template, per-run templates, and per-bug run
folders. Where `.materia/docs/specs/` tracks feature work, `.materia/docs/bugs/` tracks
...
The maintainers also file here for oversized sweep findings:
`/materia:janitor` for behavioral faults its code sweep can't fix safely
(`source: janitor`), and the UI maintainers `/materia:curator` /
`/materia:concierge` for what they surface while sweeping the live app's
...
`.materia/docs/bugs/_reports/` through the full fix pipeline and opens a PR:
...
Each run creates a `.materia/docs/bugs/<dated-slug>/` folder on a `fix/<slug>` branch and
seeds it from the templates above. At terminal state (finalize) the report file is
removed from `.materia/docs/bugs/_reports/` in the same PR.

### Per-run folder schema

Every `/materia:fix-bug` run produces a `.materia/docs/bugs/<dated-slug>/` folder with these five
artifacts:
