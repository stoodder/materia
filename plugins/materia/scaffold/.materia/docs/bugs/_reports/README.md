...
| `curator` | `/materia:curator` | Oversized behavioral findings from a live-app **visual** sweep — a UI issue too behavioral to fix in the presentation-layer envelope (§ Maintainer lifecycle § Oversized findings, [`../../standards/skills.md`](../../standards/skills.md)); the bounded visual drift is fixed directly in the curator's own PR. |
| `concierge` | `/materia:concierge` | Oversized behavioral findings from a live-app **experience** sweep — a flow / interaction-state / a11y issue too behavioral to fix in the presentation-layer envelope (§ Maintainer lifecycle § Oversized findings, [`../../standards/skills.md`](../../standards/skills.md)); the bounded rest is fixed directly in the concierge's own PR. |
| `ui-inspection` | *(retired)* | Legacy key — carried only by reports still pending in the queue from before the UI-maintainer retrofit; superseded by the UI maintainers `/materia:curator` and `/materia:concierge`. No skill writes it anymore. |
