...
[`.materia/docs/specs/`](../specs/README.md) and [`.materia/docs/bugs/`](../bugs/README.md)
...
[`.materia/docs/research/README.md`](../research/README.md) § Authoring a note —
...
- **`status`** — `active` while any member spec is unshipped; `shipped` when
  ...
  proposals are deleted from `.materia/docs/specs/_proposed/` in the same change).
...
shipping). "Shipped as" holds the `.materia/docs/specs/<dated-slug>/` folder path once
one exists.>
...
- **Member → epic + siblings, while queued (frontmatter):** each member
  proposal in `.materia/docs/specs/_proposed/` carries two extra frontmatter keys on top of the
...
- **Member → epic + siblings, forever (body):** each member proposal ends
  ...
  shipped `.materia/docs/specs/<dated-slug>/spec.md` — the durable spec-side backlink.
...
1. `/materia:propose-epic` runs brainstorm + research with the operator, then lands
   the epic folder **and** its member proposals (in `.materia/docs/specs/_proposed/`, with
...
| Writer | May write |
|---|---|
| `propose-epic` | New epic folders; new member proposals in `.materia/docs/specs/_proposed/` (`source: epic`) |
...
`.materia/docs/specs/_proposed/` rule that producers don't touch other producers' files: the two
