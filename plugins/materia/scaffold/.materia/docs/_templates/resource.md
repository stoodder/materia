<!-- template:
  Copy this to .materia/docs/resources/<entity>.md and fill it in, following
  .materia/docs/standards/docs.md (present-state only, short cells, one home per fact).
  The layer sections below are the typical full-stack set — KEEP ONLY the
  layers this repo actually has (.materia/docs/standards/architecture.md names them)
  ...
  existing doc under .materia/docs/resources/ as the local example, if one exists.
  Delete these comments. After writing: add the doc to the table in
  .materia/docs/README.md and run the checks (see .materia/docs/contributing.md).
-->
