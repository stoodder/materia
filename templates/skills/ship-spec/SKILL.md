...
|---|---|
| `/ship-spec` (no args) | Enter the **menu** — list pending proposals and ask the operator to pick. |
| `/ship-spec <id>` matching a frontmatter id under `docs/specs/_proposed/*.md` | Skip the menu, resolve to that proposal, advance. `Proposed-id-selection: named-arg` in STATUS.md. |
| `/ship-spec <slug>` matching a Resume case | Handled by the Resume gate; never reaches selection. |
| `/ship-spec` with a body of raw spec text | **Ad-hoc fallback** — treat the text as today's freeform spec. `Provenance` block filled with `—` so `finalize` skips the dequeue. |
...
(`/suggestions-to-specs`, hand-write a proposal per
...
- **Positive:** spawn `reconcile-epic` in **pipeline mode** (its
...
- **Failure (degrade, don't block):** retry a crashed/empty return once; on
  ...
  `reconcile-epic: failed — run /reconcile-epic <epic-id> standalone after
...
— `triage-retros` aggregates them and `apply-pipeline-improvements` applies
the resulting plan.
