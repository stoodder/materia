...
# materia-fix-bug — the bug-fix orchestrator

Drives a bug report to a merged PR the way `materia-ship-spec` drives a proposal —
...
See `materia-ship-spec/SKILL.md` § "Frontmatter parser" for the normative BOM-aware,
...
1. **reproduce-bug** (`materia-reproduce-bug`) → `reproduction.md` + failing test(s).
   **RED gate:** the orchestrator MUST verify `STATUS.md` stage 1 is ticked
   before advancing to stage 2 (see § RED gate).

2. **bug-analysis** (`materia-bug-analysis`) → `bug-analysis.md`. Spawned only after
   the RED gate passes.

3. **plan-tasks** (`materia-plan-tasks`) → `tasks.md`. See § plan-tasks input
   substitution for the exact spawn prompt.

4. **implement** (`materia-implement-task`, once per task, dependency order; independent
...
spawning `materia-bug-analysis`.** The check is machine-readable from the `STATUS.md`
...
**On `Blocker:`:** if `materia-reproduce-bug` wrote `Blocker:` to `STATUS.md` instead
...
advance to `materia-bug-analysis`.

Two and only two `Blocker:` exits from `materia-reproduce-bug`:
...
`materia-plan-tasks` receives `bug-analysis.md` in place of `architecture.md` as its
decomposition source. No edit to `materia-plan-tasks/SKILL.md` is needed — the
orchestrator parameterises the path in the spawn prompt.

**The exact spawn prompt text to pass to `materia-plan-tasks`:**
...
1. **`bug-analysis.md` must contain an "Affected files" section** (the
   "Affected existing resources" analogue) — otherwise `materia-plan-tasks` step-3
...
2. **Every task in the emitted `tasks.md` that touches code-under-test must
   ...
   `materia-plan-tasks` to add this AC (it's a content instruction, not a behavior
   change — `materia-plan-tasks` already writes testable ACs).
3. **`materia-plan-tasks`'s non-product-feature defaults apply unchanged.** For a bug
...
`materia-reproduce-bug`, `materia-bug-analysis`), with availability checked per
`MATERIA.md` § Tiers § Model set and the fallback per `MATERIA.md` § Tiers
§ Fallback.

## Fresh-context reviewer spawning

See `materia-ship-spec/SKILL.md` § "Fresh-context reviewer spawning" — the rule is
identical: subagents cannot spawn subagents, so all reviewers and the
docs-audit are **orchestrator-spawned**.

## Subagent liveness

See `materia-ship-spec/SKILL.md` § "Subagent liveness (long-running spawns)" — the
poll-don't-block and stall-as-resumable-failure rules are identical.

## Review (post-implementation)

See `materia-ship-spec/SKILL.md` § "Review (post-implementation)" — the six angles
...
Why: `materia-finalize`'s step 4' only recognizes the spec-pipeline `## Provenance`
...
(pointing into `docs/bugs/_reports/`) instead, so `materia-finalize`'s own step 4'
finds no spec proposal and skips silently. Relying on it would leave the report
un-dequeued. Keeping `materia-finalize` unforked means the orchestrator supplies the
...
And must instruct `materia-finalize` to perform the dequeue **after** its gate (step 3)
...
See `materia-ship-spec/SKILL.md` § "Course corrections (mid-pipeline)" — the
...
See `materia-ship-spec/SKILL.md` § "Retrospective capture (per-run `retro.md`)" —
...
See `materia-ship-spec/SKILL.md` § "Guardrails (don't spin forever)" — the bounded
...
- **Does NOT** replace `materia-ship-spec` — bugs and features stay distinct loops
...
- **Does NOT** modify `materia-triage-retros` or any of the reused stages — it
  only changes the *inputs* a bug run feeds them.
- **Does NOT** change the behavior of `materia-plan-tasks`, `materia-implement-task`, the
  review angles, `materia-docs-sync`, or `materia-finalize` beyond passing them different input
...
- **RED-before-fix invariant:** the orchestrator never advances past
  `materia-reproduce-bug` until `STATUS.md` stage 1 is ticked. This is the one
