...
`.claude/skills/ship-spec/resources/spawn-contract.md` (Block 1 always; Block
...
`.claude/skills/ship-spec/resources/env-preflight.md` applies before the
...
Every fresh `/fix-bug` invocation begins here (after the Resume gate has
...
|---|---|
| `/fix-bug` (no args) | Enter the **menu** — list pending reports and ask the operator to pick. |
| `/fix-bug <id>` where `<id>` matches a frontmatter `id` under `docs/bugs/_reports/*/report.md` | Skip the menu, resolve to that report, advance. |
| `/fix-bug <slug>` matching a Resume case | Handled by the Resume gate above; never reaches selection. |
...
You can:
  - Run /report-bug to capture a new reproducible bug report, or
...
6. **finalize** (`finalize`) → docs-sync, gate, dequeue report, PR opened.
...
spawn-prompt content override, not a fork — `plan-tasks/SKILL.md` is untouched.
(The other reused stage skills don't collide: `finalize` is stage 6 in both
...
`finalize` runs its normal docs-sync → behavior re-check → gate → PR flow
...
- Stage-id vocabulary for entries extends to: `reproduce-bug`, `bug-analysis`,
  `plan-tasks`, `implement-task:T<n>`, `finalize`, `orchestrator (pipeline-level)`.
