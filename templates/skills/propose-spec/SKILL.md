...
(the shared contract) and `.claude/skills/intake-spec/SKILL.md` § Detect
...
|---|---|
| `/propose-spec <idea text>` | Use the trailing text as the idea, advance. |
| `/propose-spec` (no args) + AskUserQuestion available | Ask "What would you like to propose? Rough is fine — I'll fill in the rest from project context." |
| `/propose-spec` (no args) + Auto Mode (no AskUserQuestion) | Print the same prompt and end the turn. The next user reply is the idea. |
...
| `## Users & context` | Single-user, on a phone, in-gym, mid-workout — this is the project's universal default. Override only if the idea is operator-tooling (e.g. a CLI). |
...
5. **Open the PR** with `gh pr create`. Title summarises the run:
   ...
   - Multi: `propose: <N> proposals from /propose-spec`.
   ...
   "Build any of them with `/ship-spec <id>`."
...
source_refs:
  - "user request via /propose-spec on <YYYY-MM-DD>"
...
- Does NOT run `ship-spec` or implement any product change. After the
  PR lands, the operator runs `/ship-spec <id>` to build it.
...
- **Always ends in a PR.** `approve` is a single, unconditional path:
  ...
  unless asked": invoking `/propose-spec` and replying `approve` **is** that
  ask.
