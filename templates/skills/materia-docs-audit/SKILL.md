---
name: materia-docs-audit
description: Verify docs against the branch's code diff after `materia-docs-sync` edits — audit coverage, accuracy, consistency, authoring-standard conformance (docs/standards/docs.md), and the mechanical `node scripts/check-docs.mjs` gate (links + style); return HIGH/MEDIUM/LOW findings or a clean verdict. Spawned by `materia-ship-spec` as a sibling stage after `materia-docs-sync` (never by a subagent). Stage 9 of the ship-spec pipeline.
---

# materia-docs-audit — verify docs against the final branch state

`materia-docs-audit` is the verify half of the self-contained doc loop that runs
between the review pass and `materia-finalize`. It runs after `materia-docs-sync` has committed
...
`materia-docs-sync` with the findings appended; if clean it proceeds to `materia-finalize`.

Runs as a **fresh-context subagent** spawned by `materia-ship-spec` — never by
`materia-docs-sync` or any other subagent (subagents cannot spawn subagents).

## Inputs

- The full branch diff vs `main` (`git diff main...HEAD`).
- The post-edit working tree (staged doc edits from `materia-docs-sync`).
- `docs/contributing.md` (the **touch-X→update-Y** map — the authoritative
  definition of what "docs" means for a given change; `materia-docs-audit` verifies
  coverage *against* it, never re-derives it from scratch).
- `docs/standards/docs.md` (the **authoring standard** — check 5 judges
  `materia-docs-sync`'s edits against it).
- The relevant `docs/**` files as they stand post-`materia-docs-sync` edits.
...
- The `materia-docs-sync` subagent's commit messages.
- `.claude/review-logs/**` (gitignored anyway; never read it).
- The prior `materia-docs-sync` agent's reasoning or intermediate steps.

## Outputs

- A HIGH/MEDIUM/LOW findings list returned to the orchestrator, or a clean
  verdict. Structured-finding schema (same as used in `materia-ship-spec` § Review):
...
- **No STATUS.md tick, no commit** — _unless the orchestrator's invocation
  ...
  § Inputs), in which case update + commit the `materia-docs-audit` row as instructed.
...
4. **Mechanical gate** — run `node scripts/check-docs.mjs` (read-only; it verifies
   ...
   finding (`materia-finalize`'s gate also runs it; fix in the loop so the gate stays
   green).
5. **Authoring-standard conformance** — judge the docs `materia-docs-sync` edited on
...
What `materia-docs-audit` does NOT do:
...
- Does not re-derive the required-updates matrix from scratch — it trusts
  `materia-docs-sync`'s matrix and verifies coverage against it.
- Does not spawn further subagents — spawned by the `materia-ship-spec` orchestrator
  only; never by `materia-docs-sync` or any other subagent.
