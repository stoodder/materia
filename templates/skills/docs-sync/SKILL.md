...
stage (after the review pass, paired with the sibling `docs-audit` stage, before
...
`pnpm run check:docs` mechanically enforces the checkable subset (narration
...
6. **Run `pnpm run check:docs`, then commit and hand off to `docs-audit`.**
   ...
   a failure here costs a full audit round (or a red `finalize` gate) later. Then commit the doc
   edits and push to the branch (`docs(<slug>): sync (round R)`; include the
   doc-change summary in the commit body). Return to `ship-spec`; the
   orchestrator spawns the sibling `docs-audit` stage next. The audit checks
   (coverage, accuracy, consistency, authoring-standard conformance, the
   mechanical gate) are performed by `docs-audit` — see
   `.claude/skills/docs-audit/SKILL.md`.

7. **Address audit findings.** The orchestrator (`ship-spec`) hands the
   `docs-audit` findings back when re-invoking `docs-sync` for round 2;
   address them and commit updated edits. The orchestrator then re-spawns
   the `docs-audit` stage. The **orchestrator-owned loop** bounds this at
   ≤2 rounds (the authoritative statement lives in `ship-spec` § Pipeline);
...
- `pnpm run check:docs` **passes — run it yourself in step 6** (links + style;
...
`pnpm run check:docs` to catch link + style issues mechanically, or invoke the
