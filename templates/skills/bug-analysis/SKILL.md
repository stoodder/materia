...
`/fix-bug` after stage 1 (reproduce-bug) is confirmed RED; usable standalone
...
- `docs/bugs/<dated-slug>/bug-analysis.md` filled per
  ...
  **`bug-analysis.md` is the `architecture.md` analogue for the bug loop.**
  It is the single document `plan-tasks` decomposes; `/fix-bug` passes it to
...
runs without the `/fix-bug` orchestrator:
