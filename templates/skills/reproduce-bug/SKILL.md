...
run proves the bug is genuine. Runs as a subagent dispatched by `/fix-bug`;
...
The fix is written by `implement-task` (stage 4), guided by `bug-analysis`
(stage 2) and `plan-tasks` (stage 3). This stage's job ends once the RED
...
(with `STATUS.md` seeded), this skill runs without the `/fix-bug` orchestrator:
...
3. Output: one or more `.spec.ts` files + `reproduction.md` — ready for
   `bug-analysis` to consume.

The next stage is `bug-analysis`.
