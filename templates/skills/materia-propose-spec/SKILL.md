...
# materia-propose-spec — capture an idea, produce a proposal file
...
models/routes), recommend [`materia-propose-epic`](../materia-propose-epic/SKILL.md)
...
is thin. `materia-intake-spec`'s structured-input detector looks for an H1 plus
...
heading itself MUST be present so `materia-intake-spec`'s detector matches.
