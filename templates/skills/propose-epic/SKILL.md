...
`.claude/skills/intake-spec/SKILL.md` § Detect the input shape (the
...
|---|---|
| `/propose-epic <idea text>` | Use the trailing text as the idea, advance. |
| `/propose-epic` (no args) + AskUserQuestion available | Ask "What's the epic about? Rough is fine — we'll develop it together." |
| `/propose-epic` (no args) + Auto Mode | Print the same prompt and end the turn. The next reply is the idea. |
...
one outcome, ≤ ~5 user stories), say so and recommend `/propose-spec`
...
4. Push and open the PR (lifecycle tooling rules). Title:
   ...
   `/ship-spec <id>` — each member's `epic:` key makes ship-spec sync the
   epic + cascade the pending siblings in that member's own PR (the epic
   gate); `/reconcile-epic <epic-id>` is the standalone backstop."
...
While this proposal is queued, /reconcile-epic may revise it if an
...
- Does NOT build anything. Members ship one at a time via
  `/ship-spec <id>`, in dependency order, at the operator's pace.
