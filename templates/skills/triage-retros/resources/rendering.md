  ...
  "actions": [
    {
      ...
      "skill": "intake-spec",
      "files": [".claude/skills/intake-spec/SKILL.md"],
...
a pure hand-off to `suggestions-to-specs`); `bugs[]` is the defect bucket
(gathered into `bug-reports.md` for `bugs-to-reports` to file). A
...
pointer to run `/bugs-to-reports`, and the `<filled by PR open>` placeholder).
...
intro paragraph (gathered, not yet filed — run `/bugs-to-reports`) ·
`## Filed reports` table, one row per bug in `bugs[]` order. The
"Report file" cell is **always `—`** — this skill mints no ids and writes no
report files; `bugs-to-reports` mints both. Source-retro cell uses the
