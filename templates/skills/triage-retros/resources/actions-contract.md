...
between `triage-retros` (which renders `pipeline-improvements.md`) and
`apply-pipeline-improvements` (which parses it, applies its `## Actions`, and
renames it `pipeline-improvements.processed.md` on consumption).
`suggestions-to-specs` and `bugs-to-reports` parse the sibling hand-offs whose
...
| `.claude/skills/ship-spec/SKILL.md` § Retrospective capture | The retro-generation contract — what future retros contain. |
| This file (`triage-retros/resources/actions-contract.md`) and `triage-retros/resources/rendering.md` | The artifact contracts the executor, `suggestions-to-specs`, and `bugs-to-reports` parse. |
