<!-- Produced by `triage-retros` and consumed by `bugs-to-reports`.
     Written iff `bugs.length > 0`; empty batch → no file.
-->

---
schema_version: 1
slug: <slug>
generated_at: <generated_at>
source_plan: docs/specs/_improvements/<slug>/pipeline-improvements.md
bug_count: <bug_count>
---

# <Slug-as-Title-Case> — bug reports

These are defect items gathered during the same synthesis pass that produced
→ pipeline-improvements.md. They are not yet filed — run `/bugs-to-reports` to file each item as a conformant report in the bug queue.

## Filed reports

| Report file | Title | Severity | Source retro |
|---|---|---|---|
| — | <title> | <severity> | `<retro_path>` § `<anchor>` |

---

_Captured by `triage-retros` run `<slug>` on `<generated_at>`._
