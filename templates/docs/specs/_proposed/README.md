...
| `log-triage` | [`logs-to-specs`](../../../.claude/skills/logs-to-specs/SKILL.md) | The running app's container logs (`docker compose logs`); triages real bugs from dev-only noise and opens a PR. Evidence snapshots live under `_proposed/_log-triage/` (producer bookkeeping) |
