...
|---|---|---|
| `retro-suggestions` | [`materia-suggestions-to-specs`](../../../.claude/skills/materia-suggestions-to-specs/SKILL.md) | `docs/specs/_improvements/**/product-suggestions.md` files emitted by `materia-triage-retros` |
| `user-proposed` | [`materia-propose-spec`](../../../.claude/skills/materia-propose-spec/SKILL.md) | The user's raw idea, refined via in-conversation Q&A; Q&A is in-memory, then on approve it branches, commits, and opens a PR |
| `janitor` | [`materia-janitor`](../../../.claude/skills/materia-janitor/SKILL.md) | Legacy key — carried only by entries still pending in the queue; the janitor is now a maintainer that fixes drift directly and writes no new queue entries |
| `ui-inspection` | [`materia-ui-inspection`](../../../.claude/skills/materia-ui-inspection/SKILL.md) | Operator-requested variant of a ui-inspection run: judgment-based UX-improvement recommendations that exceed the bug-report rubric become spec proposals; `source_refs` points at the run's report folder in `docs/bugs/_reports/` (the standards-violation findings stay in that bug report) |
| `epic` | [`materia-propose-epic`](../../../.claude/skills/materia-propose-epic/SKILL.md) | The operator's large multi-spec idea, developed via iterative brainstorm + web research into an epic under [`docs/epics/`](../../epics/README.md); each member proposal carries extra `epic:` + `depends_on:` frontmatter keys and a closing `## Epic context` body section per that contract. While a member is queued, [`materia-reconcile-epic`](../../../.claude/skills/materia-reconcile-epic/SKILL.md) (the same producer family; sanctioned in `docs/epics/README.md` § Who writes what) may revise its body + `depends_on` to track shipped siblings — the one exception to § Producer responsibilities' no-touch rule |
