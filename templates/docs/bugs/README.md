...
| [`_templates/bug-report.md`](_templates/bug-report.md) | **Body template** — the 13-section fill-in stub that `/report-bug` (and hand-authors) fill in |
| [`_templates/status.md`](_templates/status.md) | **Bug-run STATUS template** — the resumable-state stub `/fix-bug` seeds when it mints a new bug-run folder; carries the `## Bug-report provenance` block and the eight-stage checklist |
...
## /fix-bug — the consumer

`/fix-bug` is the orchestrator that drives a selected bug report from
...
Every `/fix-bug` run produces a `docs/bugs/<dated-slug>/` folder with these five
artifacts:

| Artifact | Produced by | Role |
|---|---|---|
| `STATUS.md` | `/fix-bug` (stake-and-mint) | Resumable run state — stages, provenance, per-task review state |
| `retro.md` | `/fix-bug` (stake-and-mint) | Per-stage retrospective entries; harvested by `triage-retros` |
