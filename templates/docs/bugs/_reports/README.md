...
`/report-bug` produces (and that the fill-in stub at
...
source: bug-report                          # the producer key for /report-bug
...
- **`source`** — short kebab-case identifier of the producing loop. The only
  value `/report-bug` writes is `bug-report`. New sources are added by
  convention; no enum is enforced.
- **`severity`** — a **required** closed enum: `low | medium | high | critical`.
  Gives the downstream `/fix-bug` consumer a stable field to filter and
...
report in the format `/report-bug` produces — see
...
2. An operator reviews the report in place. They either:
   - **Fix** — run the report through `/fix-bug`. `/fix-bug` reads the selected
...
A consumer (the operator triaging in place; `/fix-bug` reading the folder)
MUST:
...
`/fix-bug` is the orchestrator that drives a selected report through the full
TDD fix pipeline.
