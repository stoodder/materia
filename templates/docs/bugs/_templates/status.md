<!-- The pipeline's resumable state for this bug run. `/fix-bug` reads this to
...
<!-- Filled by `/fix-bug` at report selection (the run's entry point).
     `/fix-bug` reads `Bug-report:` to build the `finalize` spawn prompt, which
